...
# define bendl std::endl; }
  ...
  void buffer::list::iterator::copy_in(unsigned len, const list& otherl)
  {
    if (p == ls->end())
      seek(off);
    unsigned left = len;
    for (std::vector<ptr>::const_iterator i = otherl._buffers.begin();
  ...
  bool buffer::list::contents_equal(const ceph::buffer::list& other) const
  {
    ...
    if (true) {
      std::vector<ptr>::const_iterator a = _buffers.begin();
      std::vector<ptr>::const_iterator b = other._buffers.begin();
  ...
  {
    for (std::vector<ptr>::const_iterator it = _buffers.begin();
	 it != _buffers.end();
	 ++it)
      if (!it->can_zero_copy())
	return false;
  ...
  {
    for (std::vector<ptr>::iterator it = _buffers.begin();
	 it != _buffers.end();
	 ++it)
      it->zero();
  }
  						   ...
  						   unsigned align_memory)
  {
    ...
    std::vector<ptr>::iterator p = _buffers.begin();
...
//    _buffers.splice(_buffers.end(), bl._buffers );
    std::move(bl._buffers.begin(), bl._buffers.end(), _buffers.end());
    bl.clear();
    bl._len = 0;
    bl.last_p = bl.begin();
  }
...
//    _buffers.splice(_buffers.begin(), bl._buffers );
    std::move(bl._buffers.begin(), bl._buffers.end(), _buffers.begin());
    bl._len = 0;
    bl.last_p = bl.begin();
  }
...
//    tracepoint(osd, bl_append, 2);
  ...
  {
    _len += bl._len;
    for (std::vector<ptr>::const_iterator p = bl._buffers.begin();
	 p != bl._buffers.end();
	 ++p) 
...
//    _buffers.emplace_front(std::move(bp));
  ...
  const char& buffer::list::operator[](unsigned n) const
  {
    ...
    for (std::vector<ptr>::const_iterator p = _buffers.begin();
	 p != _buffers.end();
	 ++p) {
      if (n >= p->length()) {
	n -= p->length();
  ...
  char *buffer::list::c_str()
  {
    ...
    std::vector<ptr>::const_iterator iter = _buffers.begin();
    ++iter;
  ...
  char *buffer::list::get_contiguous(unsigned orig_off, unsigned len)
  {
    ...
    std::vector<ptr>::iterator curbuf = _buffers.begin();
  ...
  void buffer::list::substr_of(const list& other, unsigned off, unsigned len)
  {
    ...
    // skip off
    std::vector<ptr>::const_iterator curbuf = other._buffers.begin();
    while (off > 0 &&
	   off >= curbuf->length()) {
      // skip this buffer
  ...
  void buffer::list::write(int off, int len, std::ostream& out) const
  {
    ...
    for (std::vector<ptr>::const_iterator it = s._buffers.begin();
	 it != s._buffers.end(); 
	 ++it)
      if (it->length())
...
int buffer::list::write_fd(int fd) const
{
  ...
  std::vector<ptr>::const_iterator p = _buffers.begin();
  while (p != _buffers.end()) {
    if (p->length() > 0) {
...
{
  for (std::vector<ptr>::const_iterator p = _buffers.begin(); p != _buffers.end(); ++p) {
    raw *r = p->get_raw();
    if (r) {
      r->invalidate_crc();
    }
...
std::ostream& buffer::operator<<(std::ostream& out, const buffer::list& bl) {
  ...
  std::vector<buffer::ptr>::const_iterator it = bl.buffers().begin();
