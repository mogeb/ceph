...
int librados::RadosClient::wait_for_osdmap()
{
  ...
  if (need_map) {
    ...
    mono_clock::duration timeout;
    if (cct->_conf->rados_mon_op_timeout > 0) {
      timeout = std::chrono::duration_cast<mono_clock::duration>(
        std::chrono::duration<double>(cct->_conf->rados_mon_op_timeout)
      );
    }
    ...
    if (objecter->with_osdmap(std::mem_fn(&OSDMap::get_epoch)) == 0) {
      ...
      while (objecter->with_osdmap(std::mem_fn(&OSDMap::get_epoch)) == 0) {
        if (timeout.count() == 0) {
          ...
          mono_clock::duration elapsed = mono_clock::now() - start;
          if (elapsed > timeout) {
