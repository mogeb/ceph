...
ssize_t RDMAConnectedSocketImpl::submit(bool more)
{
  ...
  auto fill_tx_via_copy = [this](std::vector<Chunk*> &tx_buffers, unsigned bytes,
                                 std::vector<bufferptr>::const_iterator &start,
                                 std::vector<bufferptr>::const_iterator &end) -> unsigned {
    assert(start != end);
  ...
  std::vector<bufferptr>::const_iterator it = pending_bl.buffers().begin();
  std::vector<bufferptr>::const_iterator copy_it = it;
  unsigned total = 0;
